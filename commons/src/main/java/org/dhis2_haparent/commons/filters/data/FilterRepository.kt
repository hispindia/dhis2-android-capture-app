package org.dhis2_haparent.commons.filters.data

import androidx.databinding.ObservableField
import java.util.Calendar
import javax.inject.Inject
import org.dhis2_haparent.commons.filters.AssignedFilter
import org.dhis2_haparent.commons.filters.CatOptionComboFilter
import org.dhis2_haparent.commons.filters.EnrollmentDateFilter
import org.dhis2_haparent.commons.filters.EnrollmentStatusFilter
import org.dhis2_haparent.commons.filters.EventStatusFilter
...
import org.dhis2_haparent.commons.filters.FollowUpFilter
import org.dhis2_haparent.commons.filters.OrgUnitFilter
import org.dhis2_haparent.commons.filters.PeriodFilter
import org.dhis2_haparent.commons.filters.SyncStateFilter
import org.dhis2_haparent.commons.filters.WorkingListFilter
import org.dhis2_haparent.commons.filters.sorting.SortingItem
import org.dhis2_haparent.commons.filters.workingLists.WorkingListItem
...
class FilterRepository @Inject constructor(
    ...
    private fun createDefaultTrackedEntityFilters(): LinkedHashMap<ProgramFilter, FilterItem> {
        return linkedMapOf(
            ProgramFilter.ORG_UNIT to OrgUnitFilter(
                ...
                org.dhis2_haparent.commons.filters.ProgramType.TRACKER,
    ...
    private fun createDefaultDatasetFilters(
        ...
        val datasetFilters = linkedMapOf(
            DataSetFilter.PERIOD to PeriodFilter(
                org.dhis2_haparent.commons.filters.ProgramType.DATASET,
                observableSortingInject,
                observableOpenFilter,
    ...
    private fun createDefaultHomeFilters(): LinkedHashMap<HomeFilter, FilterItem> {
        val homeFilter = linkedMapOf(
            HomeFilter.DATE to PeriodFilter(
                org.dhis2_haparent.commons.filters.ProgramType.ALL,
        ...
        if (!stagesByUserAssignment.blockingIsEmpty()) {
            val assignToMeFilter = AssignedFilter(
                programType = org.dhis2_haparent.commons.filters.ProgramType.ALL,
    ...
    private fun createGetDefaultTrackerFilter(
        ...
        if (!stagesByProgramUidAndUserAssignment.blockingIsEmpty()) {
            defaultTrackerFilters[ProgramFilter.ASSIGNED_TO_ME] = AssignedFilter(
                programType = org.dhis2_haparent.commons.filters.ProgramType.TRACKER,
                sortingItem = observableSortingInject,
                openFilter = observableOpenFilter,
                filterLabel = resources.filterAssignedToMeLabel()
            )
    ...
    private fun getEventWorkingList(program: Program): WorkingListFilter? {
        ...
        if (workingLists.isNotEmpty()) {
            workingListFilter = WorkingListFilter(
                workingLists,
                org.dhis2_haparent.commons.filters.ProgramType.EVENT,
                observableSortingInject, observableOpenFilter,
                ""
            )
        }
    ...
    private fun createDefaultGetEventFilters(
        ...
        if (!stagesByProgramAndUserAssignment.blockingIsEmpty()) {
            defaultEventFilter[ProgramFilter.ASSIGNED_TO_ME] = AssignedFilter(
                programType = org.dhis2_haparent.commons.filters.ProgramType.EVENT,
                sortingItem = observableSortingInject,
                openFilter = observableOpenFilter,
                filterLabel = resources.filterAssignedToMeLabel()
            )
