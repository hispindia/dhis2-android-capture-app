...
import org.dhis2afgamis.Bindings.profilePicturePath
import org.dhis2afgamis.Bindings.toDate
import org.dhis2afgamis.R
import org.dhis2afgamis.data.forms.dataentry.*
...
import org.dhis2afgamis.utils.Result
import org.dhis2afgamis.utils.RulesActionCallbacks
import org.dhis2afgamis.utils.RulesUtilsProviderImpl
import org.dhis2afgamis.utils.analytics.AnalyticsHelper
import org.dhis2afgamis.utils.analytics.CLICK
import org.dhis2afgamis.utils.analytics.DELETE_AND_BACK
import org.dhis2afgamis.utils.analytics.SAVE_ENROLL
...
var REG_NO: String? = "v5VT9tm9UDE"
var family_unique: String? = "RH9t0P0r7Dd"
var respondent_name: String? = "zUoEIvu6RUw"
var family_member_unique: String? = "deLnry4UDR4"
var family_member_name: String? = "Ju7MEdETxsV"
var reg_value: String? = ""
var ou_uid_path: String? = null
var ou_uid_path_name: List<String>? = null
var ou_uid_path_array: List<String>? = null

var CODE_GEN: String? = "xAfV1UKjA9P"
var ou_uid: OrganisationUnit? = null
var ou_6: OrganisationUnit? = null
var ou_6_: OrganisationUnit? = null
var ou_4: OrganisationUnit? = null
var ou_uid_label: String? = null
var ou_uid_label_: String? = null
var ou_details_: String? = null
                        ...
                        .flatMap { rowAction ->
                            when (rowAction.id()) {
                                ...
                                EnrollmentRepository.ENROLLMENT_COORDINATES_UID -> {
                                    ...
                                    if (rowAction.id() == respondent_name && getProgram().uid().equals("HhujKqJsy2V")) {

//                                        val userSettings: UserSettings = d2.settingModule().userSettings().blockingGet()
//                                        MatcherAssert.assertThat(userSettings.keyUiLocale(), Is.`is`("en"))
//                                        MatcherAssert.assertThat(userSettings.keyDbLocale(), Is.`is`("en"))
//                                        UserSettings.builder();

                                        val random_n = (0..1000000).random()
                                        reg_value = rowAction.value()
                                        ou_uid_path_array = ou_uid_path!!.split("/")
                                        ou_6 = d2.organisationUnitModule().organisationUnits().byUid().eq(ou_uid_path_array!![ou_uid_path_array!!.size - 1]).one().blockingGet()
                                        valueStore.save(family_unique.toString(), ou_6!!.code() + "-" + ou_uid_path_name!![4]!!.substring(0, 3).toUpperCase() + "-" + random_n.toString())
    ...
    private fun applyRuleEffects(
        ...
        val twoStringArray = fields.get(2).value().toString().split("_".toRegex(), 3).toTypedArray()
        ou_details_=fields.get(2).value().toString()

        if (twoStringArray[2].contains(" ")) {
            ou_uid_label = twoStringArray[2].substring(0, twoStringArray[2].indexOf(" ")).toUpperCase()
        } else {
            ou_uid_label = twoStringArray[2].toUpperCase()
        }

        ou_uid = d2.organisationUnitModule().organisationUnits().byUid().eq(twoStringArray[0]).one().blockingGet()
        ou_uid_path= ou_uid!!.path()
        ou_uid_path_name= ou_uid!!.displayNamePath()
        ou_uid_label_ = ou_uid!!.parent()!!.uid()
