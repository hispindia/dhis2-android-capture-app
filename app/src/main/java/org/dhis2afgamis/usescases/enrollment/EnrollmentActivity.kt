...
import org.dhis2afgamis.Bindings.isKeyboardOpened
import org.dhis2afgamis.R
import org.dhis2afgamis.data.forms.dataentry.DataEntryAdapter
import org.dhis2afgamis.data.forms.dataentry.DataEntryArguments
import org.dhis2afgamis.data.forms.dataentry.DataEntryHeaderHelper
...
import org.dhis2afgamis.databinding.EnrollmentActivityBinding
import org.dhis2afgamis.uicomponents.map.views.MapSelectorActivity
...
import org.dhis2afgamis.utils.FileResourcesUtil
import org.dhis2afgamis.utils.ImageUtils
import org.dhis2afgamis.utils.customviews.AlertBottomDialog
...
//            val event_details = d2.eventModule().events().byUid().eq(intent.getStringExtra(EVENT_UID)).blockingGet()[0]
//            val enrollment_ = d2.enrollmentModule().enrollments().byUid().eq(event_details.enrollment()).blockingGet()[0]
            val enrollment_ = presenter.getEnrollment()
            val tei_: String? = enrollment_!!.trackedEntityInstance()
            if (enrollment_.program() == "MeiwgtORFDJ")
            {
                startActivity(TeiDashboardMobileActivity.intent(context, tei_, enrollment_.program(), enrollment_.uid()))
            }
