...
import org.dhis2afgamis.data.search.SearchParametersModel;
...
import org.dhis2afgamis.databinding.WidgetDatepickerBinding;
import org.dhis2afgamis.uicomponents.map.geometry.mapper.EventsByProgramStage;
import org.dhis2afgamis.uicomponents.map.geometry.mapper.featurecollection.MapTeiEventsToFeatureCollection;
import org.dhis2afgamis.uicomponents.map.geometry.mapper.featurecollection.MapTeisToFeatureCollection;
import org.dhis2afgamis.uicomponents.map.mapper.EventToEventUiComponent;
import org.dhis2afgamis.uicomponents.map.model.EventUiComponentModel;
import org.dhis2afgamis.uicomponents.map.model.StageStyle;
import org.dhis2afgamis.usescases.searchTrackEntity.adapters.SearchTeiModel;
import org.dhis2afgamis.utils.ColorUtils;
import org.dhis2afgamis.utils.Constants;
import org.dhis2afgamis.utils.DhisTextUtils;
import org.dhis2afgamis.utils.NetworkUtils;
import org.dhis2afgamis.utils.ObjectStyleUtils;
import org.dhis2afgamis.utils.analytics.AnalyticsHelper;
import org.dhis2afgamis.utils.customviews.OrgUnitDialog;
import org.dhis2afgamis.utils.filters.FilterManager;
import org.dhis2afgamis.utils.granularsync.SyncStatusDialog;
import org.dhis2afgamis.utils.idlingresource.CountingIdlingResourceSingleton;
...
import static org.dhis2afgamis.usescases.teiDashboard.dashboardfragments.relationships.RelationshipFragment.TEI_A_UID;
import static org.dhis2afgamis.utils.analytics.AnalyticsConstants.ADD_RELATIONSHIP;
import static org.dhis2afgamis.utils.analytics.AnalyticsConstants.CLICK;
import static org.dhis2afgamis.utils.analytics.AnalyticsConstants.CREATE_ENROLL;
import static org.dhis2afgamis.utils.analytics.AnalyticsConstants.DELETE_RELATIONSHIP;
import static org.dhis2afgamis.utils.analytics.AnalyticsConstants.SEARCH_TEI;

public class SearchTEPresenter implements SearchTEContractsModule.Presenter {
    private String test_v3;
    ...
    public void onEnrollClick() {
        ...
        if (view.fromRelationshipTEI()==null&&selectedProgram.uid().equals("MeiwgtORFDJ"))
        {
            Log.d("not-allowed","na");
        }
        else
        {
            if (selectedProgram != null&&view.fromRelationshipTEI()!=null)
            {
                TrackedEntityAttributeValue tev3= d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityInstance().eq(view.fromRelationshipTEI()).byTrackedEntityAttribute().eq("RH9t0P0r7Dd").blockingGet().get(0);
                if (d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityAttribute().eq("deLnry4UDR4").byValue().like(tev3.value()).blockingCount()!=0)
                {
                    int tev4= d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityAttribute().eq("deLnry4UDR4").byValue().like(tev3.value()).blockingCount()+1;
                    test_v3=""+tev3.value()+"-"+tev4;
                }
                else
                {
                    test_v3=""+tev3.value()+"-1";
                }
                queryData.put("deLnry4UDR4", test_v3);
                selectedProgram=d2.programModule().programs().byUid().eq("MeiwgtORFDJ").one().blockingGet();
            ...
            else
            {
                ...
                {
                    if(d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityInstance().eq(view.fromRelationshipTEI()).byTrackedEntityAttribute().eq("RH9t0P0r7Dd").blockingGet().size()>=0)
                    {
                        TrackedEntityAttributeValue tev3= d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityInstance().eq(view.fromRelationshipTEI()).byTrackedEntityAttribute().eq("RH9t0P0r7Dd").blockingGet().get(0);
                        if (d2.trackedEntityModule().trackedEntityAttributeValues().byTrackedEntityAttribute().eq("deLnry4UDR4").byValue().like(tev3.value()).blockingCount()!=0)
                        {
