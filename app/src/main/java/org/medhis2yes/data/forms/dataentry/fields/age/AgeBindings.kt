package org.medhis2yes.data.forms.dataentry.fields.age
...
import org.medhis2yes.Bindings.toDate
import org.medhis2yes.data.forms.dataentry.fields.setInputStyle
...
fun setWarningOrError(textView: TextView, warning: String?, error: String?, isSearchMode: Boolean) {
    if (warning != null) {
        val color = ContextCompat.getColor(textView.context, org.medhis2yes.R.color.warning_color)
        textView.setTextColor(color)
        textView.visibility = View.VISIBLE
        textView.text = warning
    } else if (error != null) {
        val color = ContextCompat.getColor(textView.context, org.medhis2yes.R.color.error_color)
        textView.setTextColor(color)
        textView.visibility = View.VISIBLE
        textView.text = error
    } else if (!isSearchMode) {
        val color = ContextCompat.getColor(textView.context, org.medhis2yes.R.color.textPrimary)
        textView.setTextColor(color)
